...
# Intelligent Failover Demo Watcher
# This is a copy of the traditional watcher, updated for the intelligent runbook
# Namespace points to mcp-failover-clean instead of distributed-resilience

NS="mcp-failover-clean"
SVC="web"
...
echo "[watcher-intelligent] starting automatic failover watcher (svc/${SVC}) in ns=${NS}"
...
switch_to () {
  ...
  echo "[watcher-intelligent] switching Service to ${VUP}"
...
while true; do
  ...
  echo "[watcher-intelligent] blue_ready=${BLUE_READY} green_ready=${GREEN_READY} svc=${CUR}"
  sleep "$CHECK_EVERY"
done
